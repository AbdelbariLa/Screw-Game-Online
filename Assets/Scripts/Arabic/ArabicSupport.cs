...
{
    private bool romanNumbers = false;
    private bool applyTashkeel = true;

    public override void OnInspectorGUI()
    {
        TextMeshProUGUI tmp = (TextMeshProUGUI)target;
        ...
        if (GUILayout.Button("Arabic Fix Text", buttonStyle))
        {
            FixTextForUI(tmp, tmp.text, romanNumbers, applyTashkeel);
        }
        ...
        // Toggle for Roman Numbers
        romanNumbers = EditorGUILayout.Toggle("Roman Numbers", romanNumbers);

        // Toggle for Applying Tashkeel
        applyTashkeel = EditorGUILayout.Toggle("Apply Tashkeel", applyTashkeel);
    ...
    public static void FixTextForUI(TextMeshProUGUI tmpTextComponent, string fixedText, bool useRomanNumbers = false, bool applyTashkeel = false)
    {
        string rtlText = ArabicSupport.Fix(fixedText, applyTashkeel, useRomanNumbers);
        rtlText = rtlText.Replace("\r", ""); // the Arabic fixer Returns \r\n for every \n .. needs to be removed
